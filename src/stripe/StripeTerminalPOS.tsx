...
export function StripeTerminalPOS() {
  const [salonId] = useKV<string>("salonId", "");
    ...
    return (
      <Card className="p-4 border-muted">
        <div className="flex items-center gap-2 text-muted-foreground">
          <Warning size={20} />
          <span>Complete Stripe onboarding first to use Terminal</span>
        </div>
      </Card>
    );
  }
  ...
  async function initTerminal() {
    ...
    const term = StripeTerminal.create({
      onFetchConnectionToken: async () => {
        const { secret } = await postJSON<{ secret: string }>(
  ...
  async function takePayment() {
    try {
      ...
      const { client_secret, payment_intent_id } = await postJSON<{ client_secret: string; payment_intent_id: string }>(
        "/api/stripe/terminal/payment-intents",
        { salonId, amountCents: totalCents, currency: "usd", tipCents: 0, taxCents: 0 }
      );
      ...
      if (process.paymentIntent?.status === "requires_capture") {
        setStatus("capturing");
        await postJSON("/api/stripe/terminal/capture", { paymentIntentId: payment_intent_id });
      }
