...
export const Staff = () => {
  ...
  <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 sm:gap-3 mb-2 sm:mb-3">
    ...
    <div className={`flex ${isMobile ? 'flex-col' : 'items-center'} gap-2 sm:gap-3 ${isMobile ? 'order-2' : ''}`}>
      {["list","schedule","payroll","performance","p2","p3","p4","p6","p7","p8","d1"].map(tab=>(
        <Button key={tab} onClick={()=>setActiveTab(tab)} variant={activeTab===tab?"default":"secondary"} className={`rounded-full ${isMobile?'w-full':'px-6'} font-medium transition-all duration-200 ${activeTab===tab?"bg-primary text-primary-foreground hover:bg-primary/90":"bg-secondary/50 hover:bg-secondary"}`}>
          {tab==="list"?"Staff List":tab.toUpperCase()}
        </Button>
      ))}
    </div>
    {activeTab==="list" && (
      <div className={`${isMobile?'w-full order-1':'flex-1 flex justify-end'}`}>
        <Button onClick={()=>navigate('/staff/invite')} className={`bg-primary text-primary-foreground hover:bg-primary/90 font-semibold transition-all duration-200 hover:scale-[1.02] ${isMobile?'w-full':''}`}>
          <Plus size={18} className="mr-2"/> {isMobile?"Add Staff":"Add Staff Member"}
        </Button>
      </div>
    )}
    {activeTab!=="list" && !isMobile && <div className="flex-1"></div>}
  ...
  <TabsContent value="d1" className="mt-0">
    <div className="relative min-h-[700px] rounded-3xl border border-slate-700/40 bg-black/80 p-4 sm:p-8 shadow-[0_0_80px_rgba(0,0,0,0.9)] [perspective:2800px] [transform-style:preserve-3d] overflow-hidden">
      <div className="relative mx-auto max-w-6xl" style={{transform:"rotateX(22deg) rotateY(-5deg) scale(1.06)",transformStyle:"preserve-3d"}}>
        {d1Rows.map((row,rowIdx)=>(
          <div key={rowIdx} className="[transform-style:preserve-3d] mb-[-60px] last:mb-[-20px]" style={{transform:`rotateX(${16+rowIdx*9}deg) rotateY(${(rowIdx-1.5)*11}deg)`}}>
            <div className={`grid gap-6 sm:gap-8 [transform-style:preserve-3d] justify-items-center ${row.count===4?"grid-cols-2 lg:grid-cols-4":"grid-cols-2 lg:grid-cols-3"}`}>
              {Array.from({length:row.count}).map((_,colIdx)=>{
                ...
                return(
                  <div key={colIdx} className={`${row.height} w-full max-w-[360px] rounded-2xl bg-gradient-to-br from-slate-900 via-black to-slate-950 border border-slate-600/50 shadow-[0_50px_120px_rgba(0,0,0,1),inset_0_2px_10px_rgba(255,255,255,0.03)] [transform-style:preserve-3d] transition-all duration-300 hover:scale-110 hover:z-20 hover:shadow-cyan-900/30`} style={{transform:`translate3d(${tx}px,${ty}px,${z}px) rotateX(${rotX}deg) rotateY(${rotY}deg)`}}>
                    <div className="absolute inset-0 rounded-2xl bg-gradient-to-b from-transparent via-black
