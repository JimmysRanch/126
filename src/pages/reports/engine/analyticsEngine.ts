...
export function generateDurationOverrunTrend(
  ...
  appointments
    .filter(a => a.status === 'completed' && a.actualDurationMinutes)
    .forEach(a => {
...
export function generateRatesByReminderChart(
  ...
  const sentNoShowRate = sentData.length > 0 
    ? (sentData.filter(a => a.status === 'no-show').length / sentData.length * 100) 
    : 0
  const notSentNoShowRate = notSentData.length > 0 
    ? (notSentData.filter(a => a.status === 'no-show').length / notSentData.length * 100) 
    : 0
...
export function generateTimeToReturnDistribution(
  ...
  appointments
    .filter(a => a.status === 'completed' && a.daysToNextVisit !== undefined)
    .forEach(a => {
...
export function generateUsageByCategoryChart(
  ...
  inventoryItems.forEach(item => {
    ...
    const totalUsed = appointments.filter(a => a.status === 'completed').length * usage
...
export function calculatePendingUnpaid(
  ...
  return appointments
    .filter(a => a.status === 'completed' && !settledApptIds.has(a.id))
    .reduce((sum, a) => sum + a.totalCents, 0)
}
...
export function calculateRecoveryRate(appointments: NormalizedAppointment[]): number {
  const failed = appointments.filter(a => a.status === 'no-show' || a.lateCancelFlag)
...
export function calculateAvgLTV12m(
  ...
  appointments
    .filter(a => a.status === 'completed' && parseISO(a.serviceDate) >= cutoff)
    .forEach(a => {
...
export function calculateCommissionTotal(
  ...
  return appointments
    .filter(a => a.status === 'completed')
    .reduce((sum, a) => sum + Math.round(a.netCents * 0.4), 0)
}
...
export function calculateHourlyTotal(
  ...
  const hours = appointments
    .filter(a => a.status === 'completed')
    .reduce((sum, a) => sum + (a.actualDurationMinutes || a.scheduledDurationMinutes) / 60, 0)
  
  const avgHourlyRate = 1500 // $15/hr in cents
  return Math.round(hours * avgHourlyRate)
}
...
export function calculateMessagesSent(messages: any[]): number {
  return messages.length
}

export function calculateConfirmations(messages: any[], appointments: NormalizedAppointment[]): number {
...
export function calculateAttributedShowUps(
  ...
  return appointments.filter(a => a.status === 'completed' && a.messageAttributed).length
}
...
export function calculateAttributedRevenue(
  ...
  return appointments
    .filter(a => a.status === 'completed' && a.messageAttributed)
...
export function calculateAvgTipPercent(appointments: NormalizedAppointment[]): number {
  const completed = appointments.filter(a => a.status === 'completed' && a.netCents > 0)
  if (completed.length === 0) return 0
...
export function calculateTaxableSales(appointments: NormalizedAppointment[]): number {
  return appointments
    .filter(a => a.status === 'completed' && a.isTaxable !== false)
    .reduce((sum, a) => sum + a.netCents, 0)
}
...
export function aggregateCapacityByDayStaff(
  ...
  appointments.forEach(a => {
    ...
    if (existing) {
      ...
      if (a.status === 'no-show') existing.noShows++
    }
  })
...
export function aggregateNoShowsBySegment(
  ...
  return Array.from(groups.entries()).map(([segmentValue, appts]) => {
    ...
    const recovered = appts.filter(a => (a.status === 'no-show' || a.lateCancelFlag) && a.rebookedWithin7d).length
...
export function aggregateStaffPerformance(
  ...
  return staff.filter(s => s.isGroomer).map(s => {
    ...
    const noShows = staffAppts.filter(a => a.status === 'no-show').length
    const rebooked = completed.filter(a => a.rebookedWithin7d).length
...
export function getAgingReceivables(
  ...
  const unpaidAppts = appointments.filter(a => 
    a.status === 'completed' && !settledApptIds.has(a.id)
  )
...
export function getInventoryList(
  ...
  return inventoryItems.map(item => {
    const usedCount = appointments.filter(a => a.status === 'completed').length * (item.usagePerAppointment || 0.5)
...
export function getStaffPayrollData(
  ...
  return staff.filter(s => s.isGroomer).map(s => {
    const staffAppts = appointments.filter(a => 
      a.groomerId === s.id && a.status === 'completed'
    )
...
export function generateConfirmationLiftChart(messages: any[], appointments: NormalizedAppointment[]): ChartDataPoint[] {
