...
export function PayrollCompensation() {
  ...
  const handleExportPayrollCSV = useCallback(() => {
    ...
    link.href = url
    link.download = `payroll-extract-${new Date().toISOString().split('T')[0]}.csv`
  ...
  const handleExportDrillCSV = useCallback(() => {
    ...
    const rows = drillRows.map(r => {
      ...
      return [d.serviceDate || '', d.clientName || '', d.services?.[0]?.name || '', ((d.netCents || 0) / 100).toFixed(2), ((d.commissionCents || 0) / 100).toFixed(2), ((d.tipCents || 0) / 100).toFixed(2)]
    })
    const csvContent = [headers.join(','), ...rows.map(r => r.map(c => `"${c}"`).join(','))].join('\n')
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' })
    const url = URL.createObjectURL(blob)
  ...
  if (appointments.length === 0) {
    return (
      <ReportShell title="Payroll / Compensation" description="Staff compensation tracking" defaultTimeBasis="checkout" onShowDefinitions={() => setShowDefinitions(true)}>
        <Card className="p-8 text-center">
  ...
  return (
    <>
      <ReportShell
        ...
        description="Staff compensation tracking with detailed breakdowns"
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-green-700 dark:text-green-300">
                  Export payroll data for accounting or payroll processing
                </p>
              </div>
