...
export function NoShowsCancellations() {
  ...
  const tableData = useMemo(() => {
    ...
    return measurePerformance('aggregateNoShowsTable', () =>
      aggregateNoShowsBySegment(appointments, messages, groupBy as any)
    )
  ...
  const handleKPIDrill = useCallback((metricId: string, value: number) => {
    ...
    switch (metricId) {
      case 'noShowRate':
        rows = appointments.filter(a => a.status === 'no-show').map(a => ({ id: a.id, type: 'appointment' as const, data: a, timestamp: a.serviceDate }))
      ...
      case 'lateCancelRate':
        ...
        break
      case 'lostRevenue':
        rows = appointments.filter(a => a.status === 'no-show' || a.status === 'cancelled').map(a => ({ id: a.id, type: 'appointment' as const, data: a, timestamp: a.serviceDate }))
        title = 'Lost Revenue Appointments'
        break
      case 'recoveryRate':
        rows = appointments.filter(a => (a.status === 'no-show' || a.status === 'cancelled') && a.rebookedWithin7d).map(a => ({ id: a.id, type: 'appointment' as const, data: a, timestamp: a.serviceDate }))
  ...
  const handleHeatmapDrill = useCallback((cell: { weekday: string; hour: number; value: number }) => {
    const rows = appointments
      .filter(a => {
        ...
        return weekdays[d.getDay()] === cell.weekday && h === cell.hour && a.status === 'no-show'
      })
