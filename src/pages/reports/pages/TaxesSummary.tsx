...
export function TaxesSummary() {
  ...
  const handleKPIDrill = useCallback((metricId: string, value: number) => {
    ...
    switch (metricId) {
      case 'taxableSales':
        rows = appointments.filter(a => a.status === 'completed' && a.isTaxable).map(a => ({ id: a.id, type: 'appointment' as const, data: a, timestamp: a.serviceDate }))
        title = 'Taxable Sales'
        break
      case 'nonTaxableSales':
        ...
        break
      case 'taxCollected':
        rows = appointments.filter(a => a.status === 'completed' && (a.taxCents || 0) > 0).map(a => ({ id: a.id, type: 'appointment' as const, data: a, timestamp: a.serviceDate }))
        title = 'Tax Collected'
        break
      default:
