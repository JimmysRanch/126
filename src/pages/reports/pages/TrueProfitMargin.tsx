...
import { Warning, ArrowsClockwise, ArrowRight, Info, TrendUp, TrendDown } from '@phosphor-icons/react'
...
function checkDataCompleteness(appointments: any[], staff: any[], inventoryItems: any[]): CompletenessIssue[] {
  ...
  const appointmentsWithoutDuration = appointments.filter(a => !a.actualDurationMinutes && a.status === 'completed').length
...
export function TrueProfitMargin() {
  ...
  const tableData = useMemo(() => {
    ...
    return measurePerformance('aggregateProfitTable', () => {
      const rows = aggregateByDimension(appointments, groupBy as any)
  ...
  function calculateDurationVariance(appts: any[]): number {
