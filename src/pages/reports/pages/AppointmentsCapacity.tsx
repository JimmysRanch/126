...
export function AppointmentsCapacity() {
  ...
  const tableData = useMemo(() => {
    ...
    return measurePerformance('aggregateCapacityTable', () =>
      aggregateCapacityByDayStaff(appointments, staff, groupBy as any)
    )
  ...
  const handleKPIDrill = useCallback((metricId: string, value: number) => {
    ...
    switch (metricId) {
      ...
      case 'appointmentsCompleted':
        rows = appointments.filter(a => a.status === 'completed').map(a => ({ id: a.id, type: 'appointment' as const, data: a, timestamp: a.serviceDate }))
