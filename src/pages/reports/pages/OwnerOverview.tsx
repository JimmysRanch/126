...
export function OwnerOverview() {
  ...
  const tableData = useMemo(() => {
    ...
    return measurePerformance('aggregateTableData', () => 
      aggregateByDimension(appointments, groupBy as any)
    )
  ...
  // Handle schedule
  const handleSchedule = useCallback((config: any) => {
  ...
  const handleExportDrillCSV = useCallback(() => {
    ...
    const rows = drillRows.map(r => {
      ...
      return [
        ...
        ((data.netCents || data.totalCents || 0) / 100).toFixed(2),
        ((data.tipCents || 0) / 100).toFixed(2),
        data.status || '',
      ]
    })
