...
const sizeMatrixLabels: Record<string, string> = {
  small: "Small",
  medium: "Medium",
  large: "Large",
  giant: "XL",
...
export const buildPerformanceData = ({
  ...
  const sizeLabelsOrdered = sizeOrder.map((size) => sizeLabels[size])
  ...
  const comboRpm = Array.from(comboStats.entries())
    .map(([key, stat]) => {
      ...
      return {
        label: `${breed} ${sizeLabels[size] ?? size}`,
        rpm: stat.minutes > 0 ? stat.revenue / stat.minutes : 0,
      }
    })
    .filter((item) => item.rpm > 0)
  ...
  return {
    ...
    bottomCombos,
    matrixData: {
      cols: sizeOrder.map((size) => sizeMatrixLabels[size]),
      rows: matrixRows,
    },
  }
}
